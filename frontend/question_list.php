...
// URLパラメータからジャンル情報を取得、デフォルトは「1,1」
$question_id = isset($_GET['genre']) ? $_GET['genre'] : "1,1";

// ジャンル情報をカンマで分割して配列に格納
$values = explode(",", $question_id);
// 配列の0番目をフィールドID、1番目をジャンルIDとして格納
$field = $values[0];
$genre = $values[1];

// 該当するジャンルに関連する問題データを取得
$list_questions = $form->getQuestion_fieldgenre($field, $genre);

// メッセージがURLパラメータに含まれている場合、XSS対策としてエスケープして格納
...
<head>
    ...
    <title>問題表</title>
    <link rel="stylesheet" href="../css/question_list.css"> <!-- CSSファイルを読み込む -->
</head>
<body>
<div class="border-frame">
    ...
    <form method="GET" action="" class="content-form">
        <select id="genre" name="genre"> 
            ...
            <!-- フィールドIDとジャンルIDを基に選択された項目に「selected」属性を付与 -->
